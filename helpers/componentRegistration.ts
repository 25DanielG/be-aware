...
export async function registerComponentsWithinDirectory(dirName: string) {
	...
	entries.forEach(entry => {
		if (HBS_REGEX.test(entry)) {
			...
			let componentName = entry.substring(
				0,
				endIndex
			)
...
export async function registerSingularComponent(filePath: string) {
	...
	if (!fs.existsSync(filePath)) throw new Error(`File path ${filePath} doesn't exist!`)
	...
	const startIndex = Math.max(filePath.lastIndexOf('/') + 1, filePath.lastIndexOf('\\') + 1)
